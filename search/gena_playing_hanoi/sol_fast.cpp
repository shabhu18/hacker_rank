...
bool children_contain_end_point(unordered_map < vector<bool> ,bool> hash_table ,std::vector<bool> end_state)
{
    bool contains_results;
    if(hash_table.find(end_state)== hash_table.end())
    {
...
int num_moves(int no_of_disk,std::vector<int> input_state)
{
    ...
    //
    //pair< <std::vector<bool> ,bool > pair_for_hashtable;
    //pair_for_hashtable.first=initial_state;
    //pair_for_hashtable.second=true;
    unordered_map < vector<bool> , bool>  all_states_visited;
    all_states_visited[initial_state]=true;
    ...
    //bool check=children_contain_end_point(all_states_visited,end_state);
    int index=0;
    int flag=0;
    ...
    while(flag != 1)
    {
        ...
        //check=children_contain_end_point(all_states_visited,end_state);
        parents.clear();
        parents=children;
        children.clear();
        index++;
